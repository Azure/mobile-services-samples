...
/*global Windows:true, URL:true */
    ...
    var activatedEventregistered = false;
    ...
    takePicture: function (successCallback, errorCallback, args) {
        ...
        if (sourceType != Camera.PictureSourceType.CAMERA) {
            ...
            if (navigator.appVersion.indexOf('Windows Phone 8.1') >= 0 ) {
                ...
                // The following code is modified from the 0.3.6 Camera plugin
                // to support using the FilePicker on Windows Phone.

                // Register the event handler for Windows Phone.
                if (activatedEventregistered == false) {
                    activatedEventregistered = true;

                    Windows.UI.WebUI.WebUIApplication.addEventListener("activated", function (eventObject) {
                        // var activationKind = eventObject.detail.kind;
                        var activationKind = eventObject.kind;
                        var activatedEventArgs = eventObject.detail;

                        switch (activationKind) {
                            // Only run this code on a pickFileContinuation activation.
                            // That way, you don't have to worry about catching other activations.
                            case Windows.ApplicationModel.Activation.ActivationKind.pickFileContinuation:
                                var files = activatedEventArgs[0].files;
                                var file = files.size > 0 ? files[0] : null;
                                if (file !== null) {
                                    if (destinationType == Camera.DestinationType.FILE_URI || destinationType == Camera.DestinationType.NATIVE_URI) {
                                        var storageFolder = Windows.Storage.ApplicationData.current.localFolder;
                                        file.copyAsync(storageFolder, file.name, Windows.Storage.NameCollisionOption.replaceExisting).then(function (storageFile) {
                                            // Plugin modification.
                                            // Return the path instead of the URL
                                            successCallback(storageFile.path);
                                    ...
                                    // get out
                                    errorCallback('no file returned');
                                    return;
                                }
            ...
            if (!CaptureNS.CameraCaptureUI) {
                ...
                var captureAction = function () {
                    ...
                    // Fixing bug limitation for readAsArrayBuffer
                    // (the default filename generated by Windows Phone
                    // is not supported by FileReader.readAsArrayBuffer(),
                    // which is required to upload Blob images to Azure).
                    if (window.winphone81FileName) {
                        tempFilename = window.winphone81FileName;
                    }
                    else {
                        tempFilename = "photo";
                    }
